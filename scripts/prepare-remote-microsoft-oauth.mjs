...
try {
  ...
  const redirectUri =
    args.redirectUri ??
    `http://127.0.0.1:${port}/api/connectors/microsoft/oauth/callback`;

  if (!args.skipSync) {
    await assertLocalMicrosoftOAuthConfig(sourceEnv);
    await runNodeScript(
      [
        "scripts/sync-remote-microsoft-oauth-env.mjs",
        "--host",
        host,
        "--repo",
        repo,
      ...
      "sync remote Microsoft OAuth env"
    );
  } else {
    console.log(
      JSON.stringify(
    ...
    JSON.stringify(
      {
        ...
        nextSteps: [
          ...
          "Open the authorizationUrl printed above by verify:microsoft-oauth.",
          "Complete Microsoft consent in the browser.",
          "Run npm run verify:remote-real-agent -- --require-microsoft."
        ]
      },
      null,
      2
...
async function assertLocalMicrosoftOAuthConfig(sourceEnv) {
  const values = await readEnvFile(sourceEnv);
  const missing = ["MICROSOFT_CLIENT_ID", "MICROSOFT_CLIENT_SECRET"].filter(
...
function printHelp() {
  console.log(`Usage: npm run prepare:remote-microsoft-oauth -- [options]

Prepares the SSH remote checkout for browser-based Microsoft OAuth without
printing Microsoft client secret values:
  1. verifies local MICROSOFT_CLIENT_ID and MICROSOFT_CLIENT_SECRET exist
  2. syncs them to the remote ignored env file
  3. starts the remote API with --keep-running
  4. prints the Microsoft authorization URL from the running API
...
Options:
  ...
  --redirect-uri <uri>   OAuth redirect URI. Default:
                         http://127.0.0.1:<port>/api/connectors/microsoft/oauth/callback
  --skip-sync            Do not sync local Microsoft OAuth env to remote.
