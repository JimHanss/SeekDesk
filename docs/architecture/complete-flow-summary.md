...
1. 用户在“新对话 / 本机项目”下载并安装 Windows 或 macOS 版 SeekDesk Daemon。
2. Web 调用 `POST /api/coding/daemon-pairings`，生成 10 分钟一次性配对码和 `seekdesk://pair` 深链。
3. 桌面端通过深链或手工输入调用公开领取接口；API 原子消费 code，并签发 owner/daemonId 绑定的设备 token。
4. Electron 使用 `safeStorage` 加密 token，再由系统目录选择器选择唯一 workspace root。
5. daemon 连接 `/ws/daemon`，注册机器、平台、协议版本、能力和工作区，并持续 heartbeat、断线重连。
6. Web 轮询配对状态和 workspace 列表，自动发现并选中新工作区；随后才允许创建会话。
7. API 的 `DaemonRegistry` 按 token owner 隔离连接；目录选择和工具请求只会发送给指定 daemon。
8. daemon 离线时工作区元数据仍保留，执行返回 `daemon_offline`，不会回退到服务器目录。
...
- 配对中 API restart：内存中的短期配对码失效，用户重新生成即可；已签发设备 token 仍可验证和重连。
